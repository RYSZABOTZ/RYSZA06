...
async function startSylvesterBot() {
	const store = makeInMemoryStore({ logger: pino().child({ level: 'silent', stream: 'store' }) })
	const { state, saveCreds } = await useMultiFileAuthState('Sylvester');
	...
	const getMessage = async (key) => {
		...
		return {
			conversation: 'Halo Saya Sylvester Bot'
		}
	}
	...
	store.bind(Sylvester.ev)
	
	await Solving(Sylvester, store)
	
	naze.ev.on('creds.update', saveCreds)
	
	naze.ev.on('connection.update', async (update) => {
		const { qr, connection, lastDisconnect, isNewLogin, receivedPendingNotifications } = update
		if ((connection == 'connecting' || !!qr) && pairingCode && !Sylvester.authState.creds.registered && !pairingStarted) {
			pairingStarted = true;
			let phoneNumber;
		...
		if (connection == 'open') {
			...
			let botNumber = await Sylvester.decodeJid(naze.user.id);
	...
	naze.ev.on('contacts.update', (update) => {
		for (let contact of update) {
			let id = Sylvester.decodeJid(contact.id)
			if (store && store.contacts) store.contacts[id] = { id, name: contact.notify }
		}
	});
	
	naze.ev.on('call', async (call) => {
		let botNumber = await Sylvester.decodeJid(Sylvester.user.id);
		if (global.db?.set[botNumber]?.anticall) {
			for (let id of call) {
				if (id.status === 'offer') {
					let msg = await Sylvester.sendMessage(id.from, { text: `Saat Ini, Kami Tidak Dapat Menerima Panggilan ${id.isVideo ? 'Video' : 'Suara'}.\nJika @${id.from.split('@')[0]} Memerlukan Bantuan, Silakan Hubungi Owner :)`, mentions: [id.from]});
	...
	naze.ev.on('messages.upsert', async (message) => {
		await MessagesUpsert(Sylvester, message, store, groupCache);
	});
	
	naze.ev.on('groups.update', async (update) => {
		await GroupCacheUpdate(Sylvester, update, store, groupCache);
	});
	
	naze.ev.on('group-participants.update', async (update) => {
		await GroupParticipantsUpdate(Sylvester, update, store, groupCache);
	});
	
	setInterval(async () => {
		await naze.sendPresenceUpdate('available', Sylvester.decodeJid(Sylvester.user.id)).catch(e => {})
	}, 10 * 60 * 1000);
